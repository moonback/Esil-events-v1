...
export const AIQuoteAssistant: React.FC = () => {
  ...
  const handleAnswer = async (answer: any) => {
    ...
    if (currentStep < conversationFlow.length - 1) {
      ...
      try {
        ...
        // Display suggestions with better formatting
        addBotMessage('Voici mes suggestions personnalisées pour votre événement :');
        
        // Group suggestions by type
        const packageSuggestions = response.suggestions.filter(s => s.type === 'package');
        const productSuggestions = response.suggestions.filter(s => s.type === 'product');

        if (packageSuggestions.length > 0) {
          addBotMessage('📦 Packages recommandés :');
          packageSuggestions.forEach(suggestion => {
            const message = `• ${suggestion.name}\n  ${suggestion.reason}\n  Prix total : ${suggestion.estimated_total_price}€`;
            addBotMessage(message);
          });
        }

        if (productSuggestions.length > 0) {
          addBotMessage('🛍️ Produits individuels :');
          productSuggestions.forEach(suggestion => {
            const message = `• ${suggestion.name}\n  ${suggestion.reason}\n  Prix unitaire : ${suggestion.estimated_price_per_unit}€`;
            addBotMessage(message);
          });
        }
        
        // Display additional tips with better formatting
        if (response.additional_tips) {
          addBotMessage('💡 Conseils supplémentaires :');
          addBotMessage(response.additional_tips);
        }
        
        // Add a call to action with better formatting
        addBotMessage('Souhaitez-vous que je vous aide à créer un devis avec ces suggestions ? Vous pouvez :\n• Ajouter des produits individuellement\n• Sélectionner un package complet\n• Demander plus de détails sur un produit');
      } catch (error) {
  ...
  const renderSuggestions = () => {
    ...
    // Group suggestions by type
    const packageSuggestions = suggestions.filter(s => s.type === 'package');
        ...
        {additionalTips && (
          <div className="mt-4 md:mt-6 p-3 md:p-4 bg-blue-50 rounded-xl border border-blue-100">
            <div className="flex items-start gap-2 md:gap-3">
              ...
              <div>
                <h4 className="font-medium text-sm md:text-base text-blue-800 mb-1">Conseils supplémentaires</h4>
