...
const ProductFilters: React.FC<ProductFiltersProps> = ({
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    // Count price filters
    if (priceRange[0] > minMaxPrice[0]) count++;
    if (priceRange[1] < minMaxPrice[1]) count++;
    ...
    // Count sort filter (only if not default)
    if (sortBy !== 'name-asc') count++;
    
    return count;
  }, [priceRange, minMaxPrice, selectedColors, availability, sortBy]);
  ...
  return (
    <div className="lg:w-1/5 relative">
      ...
      <button
        ...
        className="lg:hidden fixed bottom-4 right-4 z-10 bg-violet-600 text-white p-3 rounded-full shadow-lg relative"
